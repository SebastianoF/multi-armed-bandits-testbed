...
for row in range(1, timepoints):
    ...
    stds[row, :] = stds[
        row - 1, :
    ]  # + 0.1 * np.random.choice([-1, 1], size=[1, K], p=(.2, .8))
...
for strat_n, strat in enumerate(strategies):
    ...
    average_rewards_per_method[strat_n, :] = (
        cumulative_reward_per_arm_per_strategy / num_trials_per_strategy
    )
...
print(
    f"The strategy that provided the highest total reward on the average of {num_trials_per_strategy} cases is:"
)
