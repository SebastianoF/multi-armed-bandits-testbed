...
It is true that, despite many have express their negative view about the Bourbaki's work, the Bourbachist's mathematics is still widespread if not predominant across the academic community, as well as widespreadly taught.
Very often, the over-axiomatisation of a solution to a practical problem developed outside academia, had received more recognition than the original work.

Listed below are some points in favour of this approach, also aimed at explaining its success.

% -------------------------
\subsection*{Bourbaki pros}

\begin{itemize}

    \item[$\circ$] \emph{Avoid counterexamples:} this is the core reason why the group Bourbaki have started producing their work, and the validity of this approach holds also today. The endeavour of avoiding counterexample, even if possibly vain as pointed out by \cite{mathias1992ignorance}, can provide valuable tool to have a better understanding when exploring the limits of a solution.
     
    \item[$\circ$] \emph{Exercise of rigour:} the exercises of thought and rigour that the Bourbaki school provides is a quality \emph{per se}, and it provides the reader with mathematical training. 
    
    \item[$\circ$] \emph{Uniformed language:} few mathematical approaches had a stronger impact in making the mathematical language and notation uniform across countries.

    \item[$\circ$] \emph{More publications:} the pressure of publishing is increasing as much as the difficulty in discovering new theorems and models. Writing formalisations of known theories can be a way of increasing the number publications\footnote{Same goes for publishing a comparisons between various mathematical approaches, but this topic will be developed in another paper.}. Even section~\ref{se:bourbaki_perspective} of this article, once extended under the title \emph{The multi-armed bandits for the working mathematician} would provide a good candidate for a submission.
    
    \item[$\circ$] \emph{Detachment from reality:} the advantage of this point is twofold. The first one is that there are costs involved in running experiments or collecting data. The second is that there is no objective measurement of the quality or value of the work produced, besides the reader's opinion. Needless to say the second point is an advantage only in the academic domain.
    ...
    \item[$\circ$] \emph{Inaccessible to novices:} also this point holds in favour of the Bourbaki way only in the academic domain, that can justify an increase in the length of modules and courses.

\end{itemize}

% -------------------------
\subsection*{Bourbaki cons}

Many mathematicians have express their negative view about the Bourbachist's mathematics, despite it being widespread if not predominant across the academic community. The most notable ones, other than the already cited Mathias \cite{mathias1992ignorance}, are Arnold~\cite{arnol1998teaching}, De Finetti~\cite{de2008bruno}, Lockhart~\cite{lockhart2009mathematician}, Velupillai~\cite{velupillai2012bourbaki}. Sometimes even the author in favour of the Bourbaki way like Marmier~\cite{marmier2014idea}, casts some shadow with the conclusion: \lq\lq Living mathematics is being done elsewhere in the world, in connection with new problems arising from other disciplines or other human activities\rq\rq.


\begin{itemize}

    \item[$\circ$] \emph{No new results:} there is only one notable exception to this cons that I could find. The work by Mochizuki~\cite{mochizuki2012inter} is claimed to have solved the \emph{abc conjecture}. Although, as reported by Castelvecchi~\cite{castelvecchi2015biggest} \emph{the proof is too impenetrable to be understood}.
     
    \item[$\circ$] \emph{Pedagogical failure:} ZZZ Arnold
    
    \item[$\circ$] \emph{More noise:} ZZZ   It is also interesting to notice at this point the similarity between this work and the outcome of the randomized generator of maths paper at \href{https://thatsmathematics.com/mathgen/}{thatsmathematics.com/mathgen/}.
    ...
    \item[$\circ$] \emph{Less generalizable:} while this may seems not true at first glance, the reader is invited to continue the formalisation started in section~\ref{se:bourbaki_perspective}, to see how many pages and new definitions and diagrams are needed to formalise the case of arms whose distribution varies over time. Any small change in the original problem requires days of work to keep the axiomatic system in place.
    
    \item[$\circ$] \emph{Hindrance of creativity:} if a problem is made less generalisable, also the creativity in facing other problems suffers.
    ...
    \item[$\circ$] \emph{Excessive rigour can be misleading:} besides the information overload cause by there Bourbaki method, the detachment from practical problems can lead to blunders. An example of this can be found in the medical imaging paper by Lorenzi~\cite{lorenzi2013geodesics} where a counterexample found in Milnor~\cite{milnor1984remarks}, involving the tangent space of the circle, is used to justify the claims that there is no bijective correspondence between the space of the tangent vector fields and the one of their integral curves in $\mathbb{R}^3$, that contradicts the theorem of existence and uniqueness for ODEs.
    \item[$\circ$] \emph{Pure vs. Applied dichotomy:} ZZZ The advocates of the superiority of \emph{pure mathematics}\footnote{
...
\subsection*{Pragmatic pros}


\subsection*{Pragmatic cons}
